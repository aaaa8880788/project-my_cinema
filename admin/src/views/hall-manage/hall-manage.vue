...
const editClickHandle = (row: any) => {
  ...
  if (dialogRef.value) {
    ...
    formData.value.cinema = formData.value.cinema._id;
...
const dialogConfigAdj = computed(() => {
  // 获取影院数据
  const cinemaListData = store.state.home.cinemaListData;
  const formItem = dialogConfig.formItems.find(
    (item) => item.label === "影院名"
  );
  const options = cinemaListData.map((item) => {
    return {
      label: item.name,
      value: item._id,
    };
...
const hallList = computed(() => {
  return store.state.home.hallListData;
});
const hallListTotal = computed(() => {
  return store.state.home.hallListTotal;
});
</script>
